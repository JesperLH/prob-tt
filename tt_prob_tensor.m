function [G, ES, EV, Etau] = tt_prob_tensor(X,G, D, varargin)
...
[conv_crit, max_iter, ...
    ...
    model_S, fixed_S, model_V, fixed_V, model_G, fixed_G]...
    = internal.stats.parseArgs(paramNames, defaults, varargin{:});

%% Initialize
prior_s = ones(D(end-1),1)*1e+1;
...
while iter < max_iter && dELBO > conv_crit
    ...
    if iter == 1 || model_S
        ...
        N_s = N(end);%1; %or D_M-1? or numel(X_rele)
    ...
    if iter > 1
        dCost = (cost(iter)-cost(iter-1))/abs(cost(iter-1)); % Should decrease
...
for i = 1:n_modes
    if i == 1 && i < idx
        X_contr2 = tprod(X_contr, [-1, i+1:ndims(X_contr)], squeeze(G{i}), [-1, 1]);
    elseif i < idx
        X_contr2 = tprod(X_contr, [-1,-2, 2:ndims(X_contr)-1], G{i}, [-1, -2, 1]);
    elseif i > idx
        if i == idx+1
            if idx == 1
                X_contr2 = tprod(X_contr, [1,-1, 4:ndims(X_contr)+1], G{i}, [2, -1, 3]);
            else
                X_contr2 = tprod(X_contr, [1,2,-1, 5:ndims(X_contr)+1], G{i}, [3, -1, 4]);
            end
        else
            if idx == 1
                X_contr2 = tprod(X_contr, [1,2,-1,-2, 4:ndims(X_contr)-1], G{i}, [-1, -2, 3]);
            else
                X_contr2 = tprod(X_contr, [1,2,3,-1,-2, 5:ndims(X_contr)-1], G{i}, [-1, -2, 4]);
            end
        end
    else
